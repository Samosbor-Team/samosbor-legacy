...
/datum/controller/occupations
	...
	proc/AssignRole(var/mob/new_player/player, var/rank, var/latejoin = 0)
		...
		if(player && player.mind && rank)
			...
			if((job.spawned_count < position_limit) || position_limit == -1)

				Debug("Player: [player] is now Rank: [rank], JSC:[job.spawned_count], JPL:[position_limit]")
				player.mind.assigned_role = rank
				player.mind.role_alt_title = GetPlayerAltTitle(player, rank)
				unassigned -= player
				job.current_positions = job.current_positions + 1
				job.spawned_count = job.spawned_count + 1
				return 1
	...
	proc/FreeRole(var/rank)	//making additional slot on the fly
		var/datum/job/job = GetJob(rank)
		if(job && job.spawned_count >= job.total_positions && job.total_positions != -1)
			job.total_positions++
			return 1
		return 0
	...
	proc/GiveRandomJob(var/mob/new_player/player)
		...
		for(var/datum/job/job in shuffle(occupations))
			...
			if((job.spawned_count < job.total_positions) || job.total_positions == -1)
				Debug("GRJ Random job given, Player: [player], Job: [job]")
				AssignRole(player, job.title)
				unassigned -= player
				break

	proc/ResetOccupations(var/setup = 1)
 ...
 **/
	proc/DivideOccupations(var/setup = 1)
			...
			for(var/mob/new_player/player in unassigned)
				...
				for(var/datum/job/job in shuffledoccupations) // SHUFFLE ME BABY
					...
					if(player.client.prefs.CorrectLevel(job,level))
						...
						if((job.spawned_count < job.total_positions) || job.spawned_count == -1)
