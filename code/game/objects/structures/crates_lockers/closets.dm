/obj/structure/closet
	...
	var/store_items = 1
	var/store_mobs = 1
	var/samosbor_act_triggered = 0

	var/list/will_contain

/obj/structure/closet/samosbor_act()
	..()
	if(samosbor > 10 && !samosbor_act_triggered)
		samosbor_act_triggered = 1
		spawn while(1)
			if(opened)
				close()
			else
...
/obj/structure/closet/proc/store_mobs(var/stored_units)
	...
	for(var/mob/living/M in src.loc)
		if(M.buckled || M.pinned.len)
			continue
		if(samosbor)
			to_chat(M, "<span class='warning'>Something bites you!")
			M.apply_damage(rand(10,30), BRUTE, pick(BP_ALL_LIMBS))
