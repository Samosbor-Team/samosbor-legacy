...
/obj/structure/toilet/attack_hand(mob/living/carbon/human/user as mob)
	...
	if(samosbor)
		if(!open && prob(50))
			user.apply_damage(rand(5,20), BRUTE, pick(BP_R_HAND, BP_L_HAND))
			to_chat(user, "<span class='warning'>Something from the toilet bited your hand!")

		if(prob(70))
			var/turf/simulated/location = get_turf(src)
			if(istype(location))
				location.add_blood(user)
				playsound(src, 'sound/effects/gore/blood_splat.ogg', 100, 0)
			...
			if(G.state>1)
				...
				if(open && !swirlie)
					...
					if(do_after(user, 30, src))
						...
						if(samosbor)
							GM.apply_damage(rand(15,30), BRUTE, BP_HEAD)
							to_chat(GM, "<span class='warning'>Something bites your head!")
						GM.adjustOxyLoss(1)
					swirlie = null
				//else
...
/obj/machinery/shower/samosbor_act()
	..()
	if(samosbor)
		watertemp = "boiling"
...
/obj/effect/mist/blooded
	icon_state = "mist_blooded"
...
/obj/machinery/shower/update_icon()	//this is terribly unreadable, but basically it makes the shower mist up
	...
	if(on)
		if(samosbor)
			overlays += image('icons/obj/watercloset.dmi', src, "blood", MOB_LAYER + 1, dir)
		else
		...
		if(!ismist)
			spawn(50)
				if(src && on)
					if(samosbor)
						mymist = new /obj/effect/mist/blooded(loc)
					else
	...
	for(var/thing in loc)
		...
		if(istype(AM) && AM.simulated)
			...
			if(istype(L))
				process_heat(L)
	if(samosbor)
		reagents.add_reagent("blood", reagents.get_free_space(), list("donor" = null, "blood_DNA" = null, "blood_type" = "O-", "trace_chem" = null, "virus2" = list(), "antibodies" = list()))
	else
		wash_floor()
...
/obj/structure/sink/samosbor_act()
	if(samosbor)
		icon_state = "sink_blooded"
		update_icon()
...
/obj/structure/sink/attack_hand(mob/user as mob)
	if(samosbor)
		to_chat(user, "<span class='warning'>You can't wash your hands in blood!</span>")
		return
...
/obj/structure/sink/attackby(obj/item/O as obj, mob/living/user as mob)
	...
	if (istype(RG) && RG.is_open_container())
		if(samosbor)
			RG.reagents.add_reagent("blood", min(RG.volume - RG.reagents.total_volume, RG.amount_per_transfer_from_this), list("donor" = null, "blood_DNA" = null, "blood_type" = "O-", "trace_chem" = null, "virus2" = list(), "antibodies" = list()))
		else
		...
		if(!samosbor)
			...
			to_chat(user, "<span class='warning'>You wet it in blood!</span>")
