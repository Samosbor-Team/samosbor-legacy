...
/datum/controller/process/lighting
	...
	var/datum/level_generator/LG

/datum/controller/process/lighting/proc/TestAnomaly()
	LG.GenerateLevel(10, 10, 4)
...
/datum/controller/process/lighting/setup()
	...
	for(var/i = 1, i <= config.samosbor_levels, i++)
		//warning("LIGHT SETUP GEN [i]")
		var/list/level_spawners = GenerateLevel(i+5)
		for(var/spawner in level_spawners)
			spawners += spawner

	for(var/obj/effect/kh_spawner/S in spawners)
		S.Spawn()
	...
	for(var/obj/effect/kh_spawner/S in spawners)	
		...
		for(var/obj/machinery/power/apc/A in S.APCs)
			A.init()

		for(var/obj/machinery/power/smes/SMES in S.SMESs)
			SMES.init()

		for(var/obj/machinery/power/port_gen/G in S.port_gens)
			G.initialize()

		for(var/obj/structure/cable/C in S.vertical_cables)
			//warning("DENODE [S.loc.z]")
			C.denode()
