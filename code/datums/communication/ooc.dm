...
/decl/communication_channel/ooc/do_communicate(var/client/C, var/message)
	...
	for(var/client/target in clients)
		if(target.is_key_ignored(C.key)) // If we're ignored by this person, then do nothing.
			continue
	//	var/sent_message = "[create_text_tag("ooc", "OOC:", target)] <EM>[C.rank] [C.key]:</EM> <span class='message'>[message]</span>"
		var/ooc_rank = ""
		if(target.is_preference_enabled(/datum/client_preference/ooc_rank))
			ooc_rank = C.rank
		var/sent_message = "[create_text_tag("ooc", "OOC:", target)] <EM>[ooc_rank] [C.key]:</EM> <span class='message'>[message]</span>"
		if(can_badmin)
