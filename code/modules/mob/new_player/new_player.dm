...
/mob/new_player/proc/new_player_panel_proc()
	...
	panel = new(src, "“≈–Ã»Õ¿À ¬’ƒ-13","“≈–Ã»Õ¿À ¬’ƒ-13", 210, 280, src)
...
/mob/new_player/proc/AttemptLateSpawn(rank,var/spawning_at)
	...
	log_and_message_admins("Á‡¯ÂÎ ‚ Ë„Û, Í‡Í [character.mind.assigned_role]. »‰ÂÓÎÓ„Ë&#255;: [character.religion].", character)
	qdel(src)
	...
	for(var/datum/job/job in job_master.occupations)
		if(job && IsJobAvailable(job))
			...
			if(IsJobRestricted(job, client.prefs.char_branch, client.prefs.char_rank))
				dat += "<a style='text-decoration: line-through' href='byond://?src=\ref[src];SelectedJob=[job.title]'>[job.title] ([job.spawned_count]) (Active: [active])</a><br>"
			else
				dat += "<a href='byond://?src=\ref[src];SelectedJob=[job.title]'>[job.title] ([job.spawned_count]) (Active: [active])</a><br>"
...
/mob/new_player/proc/create_character(var/turf/spawn_turf, var/surname = "")
	...
	if(!spawn_turf)
		spawn_turf = jobspawn[get_rank_pref()]
	...
	real_name = "[client.prefs.real_name] [surname]"
