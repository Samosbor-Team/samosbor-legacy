...
/mob/living/carbon/human/proc/CreateWishes()
	var/datum/wish/concentrate/concentrate_wish = new(src)
	wishes[concentrate_wish.name] = concentrate_wish

	var/round_wish = pick(possible_wishes)

	var/datum/wish/round_wish/W = new round_wish(src)
	wishes[W.name] = W

/mob/living/carbon/human/proc/SuccessWish(name)
	if(wishes.Find(name))
		var/datum/wish/W = wishes[name]
		W.success()
		return 1
	return 0

/mob/living/carbon/human/proc/CheckAndSuccessWish(name, data)
	if(wishes.Find(name))
		var/datum/wish/W = wishes[name]
		if(W.check(data))
			W.success()
...
/mob/living/carbon/human/Topic(href, href_list)
	...
	if(href_list["interaction"])
		...
		if (usr.stat == DEAD || usr.stat == UNCONSCIOUS || usr.restrained() || (world.time - last_interaction) <= 10 )
			return
