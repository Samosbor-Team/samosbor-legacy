...
/mob/living/carbon/human/proc/handle_wishes()
...
/mob/living/carbon/human/proc/HalfDecay()
	return prob(100 - (100 * (decay_time / (initial(decay_time) / 2))))

/mob/living/carbon/human/proc/handle_samosbor()
	if(decay_time > 0)
		decay_time -= samosbor SECONDS
	else
		decay_time = 0
		//src.emote("scream",1)
		new /obj/effect/decal/cleanable/samosbor_gibs(get_turf(src))
		death(1)
		transforming = 1
		canmove = 0
		icon = null
	...
	if(prob(5) && decay_time > 0.3 * initial(decay_time) / 2)
		if(prob(50))
			to_chat(src, "<span class='notice'>Что-то мне не хорошо.")
		else
			to_chat(src, "<span class='notice'>Что-то не так.")
	...
	if(decay_time <= initial(decay_time) / 2)
		if(HalfDecay())
			...
			if(istype(location))
				...
				if(prob(30))
					if(prob(50))
						visible_message("<span class='warning'>Что-то вязкое капнуло с [src]!", "<span class='warning'>Что-то вязкое капает с теб&#255;!")
						adjustBruteLoss(20)
						adjustFireLoss(20)
						src.emote("scream",1)
					else
						to_chat(src, "<span class='danger'>Ты чувствуешь как тво&#255; кожа плавитс&#255;!")
						adjustBruteLoss(20)
						adjustFireLoss(20)
						src.emote("scream",1)
				else if(prob(30))
					...
					adjustBruteLoss(10)
					adjustFireLoss(10)
	if(decay_time < 0.3 * initial(decay_time) / 2)
		ChangeToHusk()
		if(prob(20 - (20 * (decay_time / (0.3 * initial(decay_time) / 2)))))
			var/obj/item/organ/external/E = pick(src.organs)
			E.droplimb(0,DROPLIMB_EDGE,1,1)
			adjustBruteLoss(50)
			adjustFireLoss(50)
			to_chat(src, "<span class='danger'>Holy fuck! I'm decomposing!")
			src.emote("scream",1)
	...
	if(stat == DEAD)	//DEAD. BROWN BREAD. SWIMMING WITH THE SPESS CARP
		...
		if(getHalLoss() >= (species.total_health - 100))
			if(!stat)
				...
				src.visible_message("<span class='warning'><B>[src]</B> больше не сопротивл&#255;етс&#255; боли!</span>")//("<B>[src]</B> [species.halloss_message].")
			Paralyse(10)
...
/mob/living/carbon/human/handle_regular_hud_updates()
	...
	if(stat != DEAD)
		...
		if(hurtdamage)
			...
			src.noise.alpha = 20 * severity + 40
		else
			src.noise.alpha = 40
		...
		if(healths)
			if (chem_effects[CE_PAINKILLER] > 100)
				...
				switch(hal_screwyhud)
					...
					if(2)	healths.icon_state = "health7"
					...
					else //If we're using Lunahud we want the lunahud health face.
						...
						switch(mhealth)
							...
							if(50 to 60)			healths.icon_state = "health3"
							if(40 to 50)			healths.icon_state = "health2"
