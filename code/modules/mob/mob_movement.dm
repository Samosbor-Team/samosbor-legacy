...
//		return
	...
	if(isliving(mob))
		...
		if(L.incorporeal_move && !(isobserver(mob)))//Move though walls
	...
	if(isobserver(mob))
		...
		if(T.holy)
			to_chat(src, "<span class='notice'>&&5^?32!*%%@@?%@3?</span>")
			return
		if(istype(T, /turf/unsimulated/floor/rescue_base))
			mob.forceMove(GetAbove(mob))
			return
		if(istype(T, /turf/simulated/wall))
			return
		if(locate(/obj/machinery/door/airlock/lift/left, T) || locate(/obj/machinery/door/airlock/lift/right, T) || locate(/obj/machinery/door/airlock/lift/middle, T))
			return
