		...
		for(var/tx = door_x1 to door_x2)
			for(var/ty = door_y1 to door_y2)
				...
				if(checking.type == floor_type) // Don't build over empty space on lower levels.
					...
					if((tx - ux) == 1)
						newdoor = new /obj/machinery/door/airlock/lift/left(checking)
					else if((tx - ux) == 2)
						newdoor = new /obj/machinery/door/airlock/lift/middle(checking)
					else if((tx - ux) == 3)
						newdoor = new /obj/machinery/door/airlock/lift/right(checking)
					else
