...
/datum/ideology/proc/initiate_elections(var/mob/initiator, var/area/A)
	...
	for(var/mob/living/M in elections_area)
		if(M.mob_ideology == src)
			...
			to_chat(M, "<span class='alert'>У тебя есть 180 секунд на то, чтобы выбрать своего кандидата.</span>")
...
/datum/ideology/monarchism
	p_name = "Имперская Партия Центра"
...
/datum/ideology/liberalism
	p_name = "Партия свободы и прогресса"
...
/datum/ideology/libert
	p_name = "Альянс \"Человечество за свободу конкуренции\""
	description = "Анархо-капиталисты или либертарианцы. Они выступают за ликвидацию государства в пользу индивидуального суверенитета в условиях свободного рынка. Их мечта - СБ вместо Полиции, ЧВК вместо Армии, Рынок вместо обязанностей."
...
/datum/ideology/fascism
	p_name = "Коалиция партий \"Человеческая Альтернатива\" и Партии Нового Рассвета"
...
/datum/ideology/communism
	p_name = "Коммунистическая Партия"
...
/mob/living/carbon/human/verb/ideology_info()
	...
	if(src.mob_ideology)
		var/message = "<span class='notice'>Твоя идеология: <b>[src.mob_ideology.name]</b>, "
		if(src.mob_ideology.has_leader)
			message += "возглавляемая <b>[src.mob_ideology.station_leader.real_name]</b>."
		else
			message += "у которой нет лидера."
		message += "<br>[src.mob_ideology.description]</span>"
		to_chat(usr, message)
	else
		to_chat(usr, "Ты не являешься приверженцем какой-либо идеологии.")
