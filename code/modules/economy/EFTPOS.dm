...
/obj/item/device/eftpos/New()
	...
	spawn(0)
		...
		R.info += "2. Confirm that your EFTPOS device is connected to your local accounts database. For additional assistance with this step, contact Kaiserliche Kolonialgenung IT Support<br>"
...
/obj/item/device/eftpos/Topic(var/href, var/href_list)
	if(href_list["choice"])
		switch(href_list["choice"])
			...
			if("reset")
				//reset the access code - requires Kanzler/captain access
				var/obj/item/I = usr.get_active_hand()
				if (istype(I, /obj/item/weapon/card))
