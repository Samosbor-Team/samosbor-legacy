...
/mob/proc/RangedAttack(var/atom/A, var/params)
	...
	if((LASER_MUT in mutations) && a_intent == I_HURT)
...
//	if(ismob(user))
//		visible_message("[user] дружелюбно машет [src]")
	return
...
/mob/proc/scramble(var/atom/A)
	...
	if(istype(src.loc, /turf/space/))
		return
	if(((abs(A.x - src.x))>1) || ((abs(A.y - src.y))>1))
		return
	if(!A || !x || !y || !A.x || !A.y) return
	if(scrambling)
		return
...
//		if(do_after(src, 10))//spawn(10)
		if((direction == NORTH) || (direction == SOUTH) || (direction == EAST) || (direction == WEST))
			...
			if((direction == 14))
				Move(get_step(src,SOUTH)) //mda
				Move(get_step(src,EAST))
			if((direction == 18))
				Move(get_step(src,SOUTH))
				Move(get_step(src,WEST))
			if((direction == 110))
				Move(get_step(src,NORTH))
				Move(get_step(src,WEST))
		scrambling = 0
		...
		if(istype(src.loc, /turf/simulated/floor))
			src.visible_message("<font size=small color=red><b>[src]</b> ползёт по полу!</font>")
		else
			src.visible_message("<font size=small color=red><b>[src]</b> ползёт!</font>")
		if(prob(1))
			if(prob(1))
				src << ("\red немцы ползут!")
//		else
//			scrambling = 0
...
/atom/proc/middle_click_intent_check(var/mob/M)
	if(M.middle_click_intent == "kick")
		return kick_act(M)
	else if(M.middle_click_intent == "jump") //добавить steal, bite
		jump_act(src, M)
	else
		M.swap_hand()
